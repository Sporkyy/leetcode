...
// Runtime: 76 ms, faster than 67.09% of JavaScript online submissions
// Memory Usage: 35.5 MB, less than 92.71% of JavaScript online submissions
...
// Runtime: 64 ms, faster than 97.69% of JavaScript online submissions
// Memory Usage: 35.6 MB, less than 91.67% of JavaScript online submissions

/**
 * @param {number} x
 * @param {number} [revX=0]
 * @return {number} Reverse of x or 0 if int32 overflow
 */
const reverse = (x, revX = 0) =>
  0 === x
    ? -(2 ** 31) <= revX && revX <= 2 ** 31 - 1
      ? revX
      : 0
    : reverse(Math.trunc(x / 10), revX * 10 + (x % 10));
